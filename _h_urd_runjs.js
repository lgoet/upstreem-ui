/* STATISCHER START fuer response-detail -- Run-JS im Pageload-Workflow.
   GENAU so, wie er in Bubble steht, unveraendert getestet.

   Das ist der google-aio-Payload: zwei Marken in companies (Anfragenfluss und LeeUP Media), beide
   auch im Antworttext ausgezeichnet, und Mentions an zwei der elf Quellen. Die Werte sind die der
   RPC, Wort fuer Wort -- auch die Fussnotenliste am Ende des Textes und die Laufmarken bleiben
   drin, damit sichtbar ist, dass die Komponente sie selbst entfernt.

   LIVE wird der Inhalt von DATEN gegen den Bubble-Ausdruck getauscht, mit den zwei
   Sanitizer-Zeilen nach STYLEGUIDE §46. */
(function(){
  var INSTANCE_ID = "response_detail_page";

  /* Ohne den Modell-Store steht im Chip der rohe Schluessel "google-aio" ohne Logo. Auf der
     echten Seite fuellt ihn der Pageload-Workflow -- hier stehen die drei bekannten Modelle,
     damit der Schritt allein lauffaehig ist. Laeuft setUpstreemModels schon woanders, kann
     dieser Block weg. */
  var MODELLE = [
  ...
  /* Die RPC liefert eine Liste mit einem Eintrag -- genau so wird sie hier weitergegeben. */
  var DATEN = [{
    ...
    response_json: {
      meta: { run_key: "962ed0083a526434ee1a178ac7-fa1bba5ef", model_start_iso: "2026-08-18T14:04:18.373+00:00" },
    ...
    companies: [
      { name: "Anfragenfluss", rank: 1, sentiment: 77, company_id: "fd2f56a4-d11d-45a3-b10e-bd93a249b75f",
        favicon_url: "https://www.google.com/s2/favicons?domain=anfragenfluss.de&sz=64",
        brand_name_raw: "Anfragenfluss" },
      { name: "LeeUp Media", rank: 6, sentiment: 57, company_id: "88640dff-6468-471a-bee4-6c17d2ed0399",
    ...
    citations: [
      { url: "https://anfragenfluss.de/neuigkeiten/beste-agentur-photovoltaik-leads",
        ...
        mentions: [{ name: "Anfragenfluss", role: "competitor", count: 1, company_id: "fd2f56a4-d11d-45a3-b10e-bd93a249b75f",
                     match_type: null, favicon_url: "https://www.google.com/s2/favicons?domain=anfragenfluss.de&sz=64" }],
      ...
      { url: "https://www.lee-up.de/solar",
        ...
        mentions: [{ name: "LeeUp Media", role: "own", count: 1, company_id: "88640dff-6468-471a-bee4-6c17d2ed0399",
                     match_type: null, favicon_url: "https://www.google.com/s2/favicons?domain=lee-up.de&sz=64" }],
