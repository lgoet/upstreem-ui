(function(){
  ...
  /* Jeder Textwert steht in BACKTICKS: die vertragen Anfuehrungszeichen und echte Zeilenumbrueche,
     doppelte Anfuehrungszeichen nicht. Jede Liste geht durch LISTE(): kaputte Liste heisst leere
     Liste, nicht kaputte Seite. */
  function LISTE(t){
    var s = String(t == null ? "" : t).trim();
    if (!s) return [];
    try { var v = JSON.parse(s); return Object.prototype.toString.call(v) === "[object Array]" ? v : [v]; }
    catch (e) {}
    var UC = window.UpstreemCore;
    if (UC && UC.parseLoose){
      var r = UC.parseLoose(s, "response-detail (Liste)");
      if (r) return Object.prototype.toString.call(r) === "[object Array]" ? r : [r];
    }
    if (window.console) console.warn("[response-detail] eine Liste war nicht lesbar und bleibt leer.");
    return [];
  }
  ...
  var DATEN = [{
    ...
    response_json: {
      ...
      text: `Kurzer Einstiegsabsatz mit einer Marke: LeeUP Media ist dabei. [0](https://www.lee-up.de/solar) [1](https://anfragenfluss.de/neuigkeiten/beste-agentur-photovoltaik-leads)
...
[[1] - Beste Agentur für Photovoltaik-Leads - Anfragenfluss](https://anfragenfluss.de/neuigkeiten/beste-agentur-photovoltaik-leads)`
  ...
  (function los(){
    if (typeof window.setResponseDetail === "function") {
      if (MODELLE.length && typeof window.setUpstreemModels === "function") {
